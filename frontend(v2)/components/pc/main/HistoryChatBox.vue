<template>
  <div class="flex flex-col ht-vc top-line-box h-full">
    <div
      class="flex flex-row hc-vc title-bar fs14 h-full pd-5"
      :style="titleBarStyleObj"
    >
      <div class="text-center fc-white">JAPANESE</div>
      <div class="flex flex-row hc-vc leave-box pointer">
    ...
    <div
      ...
      class="flex flex-row hc-vt h-full value-box"
      :style="mainContentStyleObj"
    >
      <div class="flex flex-col hc-vc v-full">
        <div
          class="flex flex-row hc-vc tab-box v-full pointer"
        ...
        <div
          v-if="isLogIn"
          class="flex flex-row hc-vc tab-box v-full pointer visible-option"
...
export default {
  ...
  props: {
    ...
    isUserListShow: { type: Boolean, default: false },
    chatInfos: {
      type: Array,
      default() {
        return [{ name: "colly2", chatData: "I'm busy" }];
      },
    },
    historyInfos: {
      type: Array,
      default() {
        return [{ timeStamp: "00:00", historyData: "Daniel !bust100" }];
      },
    },
    userInfos: {
      type: Array,
      default() {
        return [{ name: "colly2", amp: 2.11, bet: 1000, profit: "-" }];
      },
    },
    autoBetBtnTitle: { type: String, default: "Bet" },
  },
  data() {
    return { curTabID: this.activeTabId, tabHeight: 0, isLogIn: false };
  },
  computed: {
    titletBarStyleObj: {
      get() {
        return { height: variables.historyChatBoxHeaderHeight };
      },
    },
    mainContentStyleObj: {
      get() {
        ...
        subPxs.push("" + variables.topLineBoxBorderHeight);
        return { "min-height": "calc(" + subPxs.join(" - ") + ")" };
      },
    },
    listHeightStyleObj: {
      get() {
  ...
  methods: {
    __resizeHandler() {
      let nTabCnt = 2;
      const totalHeight = this.$refs.vertical_tab_box.clientHeight;
