<template>
  <div class="score-graph-container br-sm">
...
export default {
  ...
  methods: {
    ...
    configPlotSettings() {
      ...
      this.XAxisPlotMinValue = 10000; /* 10 Sec */
      this.YAxisSizeMultiplier = 2; /** YAxis is x times */
      this.YAxisInitialPlotValue = "zero";
    },
    calcGameData() {
    ...
    drawGraph() {
      ...
      else if (this.currentGamePayout >= 15) this.ctx.strokeStyle = "#ffde00";
      else this.ctx.strokeStyle = "#ffde00";
    ...
    drawAxes() {
      ...
      this.payoutSeparation = tihs.stepValues(
      ...
      /** Calculate X Axis */
      this.millisecondsSeparation = this.stepValues(this.XAxisPlotValue, true);
      this.XAxisValuesSeparation =
        this.plotWidth / (this.XAxisPlotValue / this.millisecondsSeparation);
      /** Draw X Axis Values */
      for (
        let milliseconds = 0, counter = 0, s = 0;
        milliseconds < this.XAxisPlotValue;
        milliseconds += this.millisecondsSeparation, counter++, s++
      ) {
    ...
    drawGameData() {
      ...
      const payout =
        this.growth(this.currentTime) - 1; /** We start counting from one x */
      const y = this.plotHeight - payout * this.heightIncrement;
